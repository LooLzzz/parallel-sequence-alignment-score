...
int main(int argc, char *argv[])
{
    ...
    int ranks[8] = {2, 4, 7}, size, newSize, newRank, rank;
    ...
    if (size < 8)
    {
        printf("Test requires more than 7 processors\n");
    ...
    if ((rank == ranks[0] || rank == ranks[1] || rank == ranks[2]) && (newRank == 0))
        strcpy(msg, "Hello");
    ...
    {
        int value = 123;
        MPI_Send(&value, 1, MPI_INT, 2, 0, newComm);
    }
    else if (newRank == 2)
    {
        int data;
        MPI_Recv(&data, 1, MPI_INT, 1, 0, newComm, &status);
        printf("newRank 2 received %d from newRank 1\n", data);
    }
